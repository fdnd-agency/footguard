...
export async function sendGroupInviteEmail({ to, inviteLink, groupName }) {
  if (EMAIL_PROVIDER === 'console') {
    console.log('--- GROUP INVITE EMAIL (DEV MODE: console) ---')
    console.log('To:', to)
    console.log('From:', env.MAIL_FROM ?? '(not set)')
  ...
  if (EMAIL_PROVIDER === 'resend') {
    ...
    if (!env.MAIL_FROM) throw new Error('MAIL_FROM is missing')
    ...
    await resend.emails.send({
      from: env.MAIL_FROM,
      to,
      subject: `You have been invited to join ${groupName ?? 'a group'}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background-color: #f9f9f9; border-radius: 8px;">
          <h2 style="color: #1a1a1a; margin-bottom: 8px;">Group Invitation</h2>

          <p style="color: #444; font-size: 15px; line-height: 1.6;">
            You have been invited to join <strong>${groupName ?? 'a group'}</strong>.
          </p>

          <div style="text-align: center; margin: 32px 0;">
            <a href="${inviteLink}" style="background-color: #1a6fc4; color: #ffffff; padding: 14px 28px; border-radius: 6px; text-decoration: none; font-size: 16px; font-weight: bold;">
              Accept invitation
            </a>
          </div>
